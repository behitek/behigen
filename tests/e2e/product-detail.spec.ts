...
test('product detail page shows pricing and internal contact links', async ({
  page
}) => {
  await page.goto('/store/google-ai-pro-2tb-family-owner');

  await expect(
    page.getByRole('img', { name: /google product badge/i })
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('heading', { name: /So sánh các gói cùng nhóm/i })
  ).toBeVisible();
  await expect(
    page.getByRole('columnheader', { name: /Tiêu chí/i })
  ).toBeVisible();
  await expect(page.getByText('Gói hiện tại')).toBeVisible();
  await expect(
    page.locator('a[href*="google-ai-pro-2tb-family-member"]').first()
  ).toBeVisible();
  await expect(
...
test('product detail uses group pricing interactions and only keeps Zalo visible on mobile', async ({
  page
}) => {
  ...
  await expect(page.getByText('140K').first()).toBeVisible();
  await expect(page.getByText('700K').first()).toBeVisible();
});
