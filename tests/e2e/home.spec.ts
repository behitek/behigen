...
test('homepage shows featured products and contact choices', async ({ page }) => {
  await page.goto('/');
...
test('homepage CTA leads to the store', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('link', { name: /Xem cửa hàng/i }).first().click();
...
test('homepage promotes both practical tools and labs', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /Công cụ thực dụng và LLM Lab/i })).toBeVisible();
  await expect(page.getByRole('link', { name: /Xem gói phù hợp/i })).toHaveAttribute(
    'href',
    /\/tools\/ai-package-quiz$/
  );
});
